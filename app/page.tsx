...
export default async function HomePage() {
  ...
  const [
    ...
    { count: freeListingsCount },
    { count: profilesCount },
    { data: listingsData },
  ] = await Promise.all([
    supabase.from('listings').select('*', { count: 'exact', head: true }),
    supabase.from('listings').select('*', { count: 'exact', head: true }).or('status.neq.removed,status.is.null'),
    supabase.from('listings').select('*', { count: 'exact', head: true }).or('item_type.eq.Free,item_type.eq.free'),
  ...
  const realStats = [
    { value: `${profilesCount ?? 0}`, label: 'Registered Students' },
    { value: `${liveListingsCount ?? 0}`, label: 'Active Live Items' },
    { value: `${totalListingsCount ?? 0}`, label: 'Total Postings' },
    { value: `${freeListingsCount ?? 0}`, label: 'Free Giveaways' },
  ];
        ...
        <div className="grid items-center gap-12 lg:grid-cols-12">
          <div className="lg:col-span-7">
            ...
            <div className="mt-8 flex items-center gap-2.5 text-xs text-ink-3">
              ...
              <span>{profilesCount ?? 0} registered students • {liveListingsCount ?? 0} live items active</span>
            </div>
          </div>
          ...
          <div className="hidden lg:block lg:col-span-5">
            <div className="rounded-2xl border border-stone-light/80 bg-white p-4 shadow-2xs">
              <div className="flex items-center justify-between border-b border-stone-light/60 pb-3 mb-3">
                ...
                <span className="text-[10px] text-ink-3 uppercase tracking-wider font-mono">Realtime</span>
              </div>
      ...
      <section className="border-y border-stone-light/60 bg-white py-8">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-6 px-4 sm:px-6 lg:grid-cols-4">
          {realStats.map((s, idx) => (
            <div
              key={s.label}
              className={`text-center ${idx !== realStats.length - 1 ? 'lg:border-r lg:border-stone-light/50' : ''}`}
            >
